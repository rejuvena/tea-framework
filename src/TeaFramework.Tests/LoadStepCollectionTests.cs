...
namespace TeaFramework.Tests
{
    public static class LoadStepCollectionTests
    {
        ...
        public static void TestOrdering()
        {
            ...
            Dictionary<string, float> randomSequence = steps.OrderBy(x => rand.NextDouble()).ToDictionary(x => x.Key, x => x.Value);

            LoadStepCollection collection = new();
            for (int i = 0; i < iterations; i++)
                collection.Add(new LoadStep(randomSequence.Keys.ElementAt(i), randomSequence.Values.ElementAt(i), x => { }));

            float counter = 1f;
            foreach (LoadStep step in collection)
