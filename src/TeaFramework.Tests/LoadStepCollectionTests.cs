...
namespace TeaFramework.Tests
{
    public static class LoadStepCollectionTests
    {
        ...
        public static void TestOrdering() {
            ...
            for (int i = 0; i < iterations; i++) names.Add(Guid.NewGuid().ToString());

            List<float> weights = new();
            for (int i = 1; i < iterations + 1; i++) weights.Add(i);

            Random rand = new();
            Dictionary<string, float> steps = weights.ToDictionary(x => names[(int) x - 1]);
            Dictionary<string, float> randomSequence = steps.OrderBy(_ => rand.NextDouble())
            ...
            float counter = 1f;
            foreach (ILoadStep step in collection) Assert.AreEqual(counter++, step.Weight);
        }
    }
}